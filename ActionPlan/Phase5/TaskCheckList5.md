...
- [x] Create `base/templates/entrypoint.sh.template` as bash script
- [x] Add shebang: `#!/bin/bash` and `set -e` for exit on error
- [x] Implement config file validation: check if MCP_CONFIG_PATH exists, validate YAML syntax with `yamllint` or Node script
- [x] Add environment variable override: `export $(grep -v '^#' .env | xargs)` if .env file exists
- [x] Check required variables: verify critical env vars are set (DATABASE_URL, etc), exit with error message if missing
- [x] Implement startup logging: echo "Starting MCP Server...", log config location, list loaded tools/connectors
- [x] Add process management: handle SIGTERM to gracefully stop server, forward signals to Node.js process
- [x] Support different run modes: accept command line arg (dev/prod), adjust logging verbosity accordingly
- [x] Execute main server: `exec node server.js "$@"` to replace shell process with Node
- [x] **Success Criteria**: Validates config before starting; sets up environment; logs startup info; handles signals properly
