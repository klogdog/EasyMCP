...
- [x] Create `base/config-generator.ts` with `async function generateConfig(manifest: MCPManifest, secrets: Record<string, string>): Promise<string>`
- [x] Define config schema: database (url, pool settings), services (API keys by service name), logging (level, format), features (flags), server (port, host)
- [x] Create YAML template using js-yaml library: iterate through manifest connectors, add placeholder for each required credential
- [x] Insert environment variable references: use `${VAR_NAME}` syntax for secrets, `${PORT:-8080}` for defaults
- [x] Add inline comments explaining each section and how to override via env vars
- [x] Generate different variants: development (verbose logging, localhost), production (structured logs, external DB)
- [x] Return formatted YAML string
- [x] Include validation function: `validateConfig(configString: string): boolean`
- [x] **Success Criteria**: Generates valid YAML; includes all connector credentials; supports env var substitution
